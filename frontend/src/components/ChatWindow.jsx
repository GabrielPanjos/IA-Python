  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center px-6 animate-fade-in">
      ...
      <p className="text-sm max-w-xs leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
        Seu assistente de programação. Faça uma pergunta para começar.
      </p>

      <div className="grid grid-cols-2 gap-3 mt-8 max-w-sm w-full">
        {[
          { icon: BookOpen, label: 'Conceitos', desc: 'Aprenda fundamentos' },
          { icon: Zap, label: 'Debug', desc: 'Resolva erros' },
          { icon: MessageSquare, label: 'Exemplos', desc: 'Código prático' },
          { icon: Code2, label: 'Desafios', desc: 'Pratique aprendendo' },
...
export default function ChatWindow({ messages, isLoading }) {
  ...
  // Only show real messages (not the welcome one) for "empty state"
  ...
  if (!hasRealMessages && messages.length <= 1) {
    return <EmptyState />
  }
