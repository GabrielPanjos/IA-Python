...
const TIPS = [
  'Tente explicar o código em voz alta — o feito de falar ajuda a fixar o conceito.',
  ...
  'Commits pequenos e frequentes são melhores que um grande commit.',
]
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute -right-3 top-6 z-10 flex items-center justify-center w-6 h-6 rounded-full border transition-colors"
        style={{
      ...
      <div className="flex items-center gap-2.5 px-3 py-4 border-b" style={{ borderColor: 'var(--border-subtle)' }}>
        ...
        {!collapsed && (
          <div className="animate-fade-in overflow-hidden">
            <p className="font-bold text-sm leading-none" style={{ color: 'var(--text-primary)', fontFamily: 'Space Grotesk' }}>
              DevMentor
            </p>
            <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>AI Tutor</p>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 px-2 py-3 space-y-1 overflow-hidden">
        <button
          onClick={() => !collapsed && setShowSettings(v => !v)}
          className="flex items-center gap-2.5 w-full px-2.5 py-2 rounded-lg text-left transition-colors group"
        ...
        {!collapsed && showSettings && (
          <div
            className="mx-1 mt-1 p-3 rounded-lg animate-slide-up"
      ...
      {!collapsed && (
        <div
          className="mx-3 mb-4 p-3 rounded-lg animate-fade-in"
          style={{ background: 'rgba(86,207,178,0.06)', border: '1px solid rgba(86,207,178,0.15)' }}
        >
          <div className="flex items-center gap-1.5 mb-1.5">
            <BookOpen size={11} style={{ color: 'var(--accent-secondary)' }} />
            <span className="text-xs font-semibold" style={{ color: 'var(--accent-secondary)' }}>DICA DO DIA</span>
          </div>
